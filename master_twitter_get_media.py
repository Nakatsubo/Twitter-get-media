...
class TweetsGetter(object):
    ...
    def __init__(self):
        self.session = key.CreateOAuthSession()
...
class TweetsGetterByUser(TweetsGetter):
    ...
    def specifyUrlAndParams(self):
        ...
        params = {'screen_name':self.screen_name, 'count':200, 'include_entities':True}
        return url, params
...
if __name__ == '__main__':
    ...
    # フォルダを指定して保存（screen_name）
    foldername = './images/' + SCREEN_NAME
    ...
    for tweet in getter.collect(total = 3000):
        ...
        if 'media' in tweet['entities']:
            if 'video_info' in tweet['entities']:
                print('video\n')
            else:
                for media in tweet['entities']['media']:
                    title = foldername + '/' + tweet['id_str'] + '_' + str(cnt) + '. jpg'
